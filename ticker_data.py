...
class Ticker():
  ...
  def _read_datum(self):# gets a range of datum
    ...
    for key in ['high', 'volume', 'close', 'open', 'low']:
      for i, value in enumerate(quotes[key]):
        datum[i][key] = value
    if 'indicators' in results and 'adjclose' in results['indicators'] and len(results['indicators']['adjclose']) > 0 and 'adjclose' in results['indicators']['adjclose'][0]:
  ...
  def get_hourly_month_data(self, end=datetime.utcnow()):
    if end == None:
      end = datetime.utcnow()
    start = end - timedelta(days=30)
    start = int(mktime(start.timetuple()))
    end = int(mktime(end.timetuple()))
    self.json_data = self._call_url({"period1":start, "period2":end, "interval":'1h', "includePrePost":True, "events":"div,splits"})
    return self._read_datum()
